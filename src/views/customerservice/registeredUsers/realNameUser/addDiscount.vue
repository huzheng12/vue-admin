  ...
  <div class="add-discount">
    ...
    <el-form class="form" :rules="rules" label-position="top" :model="formData" ref="ruleForm">
      <el-form-item label="交易类型" prop="type">
        <el-select v-model="formData.type" class="isPageSelect">
          <el-option
...
export default {
  ...
  methods: {
    ...
    // 添加折扣
    async addUserfeesCustomer() {
      try {
        let customerId = this.$route.params.customerId
        // console.log(customerId);
        
        let res = await this.$api.customerservice.registeredUsers.addUserfeesCustomer({
          customerId,
          ...this.formData
        })
        // console.log(res);
