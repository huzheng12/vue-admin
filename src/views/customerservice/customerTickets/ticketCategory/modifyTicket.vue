<template>
  <div class="page">
    <!-- 标题 -->
    <div class="title">
      <span class="title-text">修改工单类别</span>
      <el-button plain size="mini" class="title-bnt" icon="el-icon-caret-left" @click="backModus">返回</el-button>
    </div>
    <!-- 表单部分 -->
    <div class="form">
      <el-form ref="form" :model="form" label-width="80px" hide-required-asterisk label-position="top" :rules="rules">
        <el-form-item label="语言" prop="region">
          <el-select v-model="form.activeName" placeholder="请选择">
            <el-option :label="item.name" :value="item.value" v-for="(item, index) in form.langList" :key="index"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item size="large">
          <el-button v-if="$filterBtnRoute(1222)" @click="submitForm('form')" type="primary">提交</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async modifysee() {
      try {
        const res = await this.$api.customerservice.viewticketcategory({
          id: this.id
        })
        // console.log('viewticketcategory', res);
        if (res.success) {
          const data = res.obj
          // const tempLang = data.languageDic
          // this.form.activeName = this.form.langList.filter(item => 
          // {
          ...
          this.form.name = data.typeName,
            ...
            this.form.activeName = data.languageDic
        }
      } catch (e) {
        this.$handleError.handleApiRequestException(e)
      }
          ...
          try {
            const res = await this.$api.customerservice
              .editticketcategory({
                ...
                languageDic: this.form.activeName, // 语言类型
