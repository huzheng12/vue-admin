<template>
  <div class="page">
    <!-- 标题 -->
    <div class="title">
      <span class="title-text">添加站内信模板</span>
      <el-button size="mini" class="title-bnt" icon="el-icon-arrow-left" @click="backModus">返回</el-button>
    </div>
    <!-- 表单部分 -->
    <div class="form">
      <el-form
      ...
      >
        ...
        <el-form-item label="模板名称" prop="name">
          <el-input v-model="form.name" resize="none" :rows="10"></el-input>
        </el-form-item>
        <!-- {{form.content}} -->
        <!-- {{form.content.length}} -->
        ...
        <div class="form-content">
          <div class="form-content-title">模板内容</div>
          <quill-editor
        ...
        <el-form-item size="large">
          <el-button @click="submitForm('form')" type="primary" v-if="$filterBtnRoute(1172)">提交</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
        ...
        if (valid) {
          ...
          try {
            const res= await this.$api.GlobalManagement.letterTemplate
              .add({
                messageCategory: this.form.region,    // 系统语种
