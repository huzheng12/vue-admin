    ...
    <div style="padding: 0 20px">
      ...
      <el-form class="form" :rules="rules" label-position="top" :model="formData" ref="ruleForm">
        <el-row justify="space-around" :gutter="30">
          <el-col :span="12">
            <el-form-item label="选择语种" prop="langCode">
              <el-select v-model="formData.langCode">
                <el-option
        ...
        <el-row justify="space-around" :gutter="30">
          <el-col :span="12">
            <el-form-item label="是否开启" prop="isStick">
              <el-select v-model="formData.status">
                <el-option :label="item.label" :value="item.value * 1" v-for="item in yesOrno" :key="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-form-item label="文章内容(必填)" prop="content">
          <!-- <vue-ueditor-wrap class="ueditor" v-model="formData.editorValue" :config="myConfig"></vue-ueditor-wrap> -->
          <quill-editor
        ...
        <el-form-item>
          <el-button type="primary" v-if="$filterBtnRoute(861)" @click="submitForm('ruleForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      // editTextInfo: '<p><img src="http://35.201.165.105:8000/storage/image/20200319/1584598736984971.png" title="1584598736984971.png" alt="poll_app.png"/></p>', // 富文本内容
      // myConfig: {
      //   // 百度富文本配置
      //   // 编辑器不自动被内容撑高
  ...
  methods: {
    ...
    // },
    // 添加文章内容
    async addApparticle(data) {
      try {
        let res = await this.$api.GlobalManagement.DraftContent.addApparticle(
          data
        )
        // console.log(res);
        if (res.success) {
